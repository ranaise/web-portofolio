...
export function Experience() {
  ...
  return (
    <section id="experience" className="experience-page-content home-experience-block">
      <header className="experience-section-heading"><p className="section-kicker"><span>02</span><span>Experience</span></p><h2>Professional experience</h2></header>
      ...
      <section className="additional-experience"><div className="organization-heading"><p className="chapter-label">Organization experience</p><p>Leadership, teamwork, and technical learning outside the classroom.</p></div><div className="organization-grid">{orgExperienceData.map((item, index) => <Reveal key={item.id} delay={index * .08} className="organization-card"><div className="organization-photo">{item.photos?.[0] && <Image src={item.photos[0]} alt={`${item.role} certificate`} fill sizes="(max-width: 767px) calc(100vw - 40px), 34vw" className="object-contain" />}</div><div className="organization-card-copy"><span className="organization-index">0{index + 1}</span><p className="chapter-label">{item.duration}</p><h3>{item.role}</h3><p>{item.organization}</p><ul>{item.tasks.map((task) => <li key={task}>{task}</li>)}</ul></div></Reveal>)}</div></section>
    </section>
  );
}

...
export function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 border-b border-border/70 bg-background/88 backdrop-blur-xl">
      <nav className="mx-auto flex h-18 max-w-[1440px] items-center justify-between px-5 sm:px-8 lg:px-12" aria-label="Primary navigation">
        <Link href="/#home" className="focus-ring font-serif text-2xl leading-none tracking-[-.04em]">RN.</Link>
        ...
        <div className="flex items-center gap-1 md:hidden">
          ...
          <Sheet><SheetTrigger render={<Button variant="ghost" size="icon" className="size-11 rounded-full" aria-label="Open menu" />}><Menu /></SheetTrigger><SheetContent className="w-[88vw] max-w-sm border-l-border bg-background p-6"><SheetHeader className="px-0 pt-8 text-left"><SheetTitle className="font-serif text-3xl">Index</SheetTitle><SheetDescription>Navigate this case file.</SheetDescription></SheetHeader><div className="mt-10 flex flex-col border-t border-border">{navigationItems.map((item, index) => <SheetClose key={item.label} render={<Link href={item.href} className="flex min-h-16 items-center justify-between border-b border-border text-xl" />}><span>{item.label}</span><span className="font-mono text-xs text-muted-foreground">0{index + 1}</span></SheetClose>)}</div></SheetContent></Sheet>
        </div>
      </nav>
    </header>
  );

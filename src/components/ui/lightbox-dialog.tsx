...
export function LightboxDialog({ isOpen, onClose, title, images, initialIndex = 0, description }: LightboxProps) {
  ...
  return <Dialog open={isOpen} onOpenChange={(open) => { if (!open) onClose(); }}><DialogContent className="grid max-h-[92svh] max-w-[calc(100%-1rem)] grid-cols-1 overflow-hidden bg-[#17130f] p-0 text-[#fffaf0] sm:max-w-6xl lg:grid-cols-[1fr_340px]">
    <div className="relative flex min-h-[55svh] items-center justify-center bg-black/35 p-4"><Image src={images[index]} alt={`${title}, image ${index + 1}`} fill priority sizes="(max-width: 1024px) 100vw, 70vw" className="object-contain p-4" />{images.length > 1 && <><Button variant="outline" size="icon" className="absolute left-3 top-1/2 size-11 -translate-y-1/2 rounded-full border-white/20 bg-black/50 text-white" onClick={() => move(-1)} aria-label="Previous image"><ChevronLeft /></Button><Button variant="outline" size="icon" className="absolute right-3 top-1/2 size-11 -translate-y-1/2 rounded-full border-white/20 bg-black/50 text-white" onClick={() => move(1)} aria-label="Next image"><ChevronRight /></Button><span className="absolute bottom-4 left-1/2 -translate-x-1/2 rounded-full bg-black/60 px-3 py-1 font-mono text-xs">{index + 1} / {images.length}</span></>}</div>

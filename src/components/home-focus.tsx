...
export function HomeFocus() { return <section className="home-section home-focus" aria-labelledby="focus-title"><header><p className="chapter-label">Focus</p><h2 id="focus-title">Technical work explained clearly</h2><p>I care about the connections that make a product feel coherent.</p><Link href="/about" className="text-action">About Rafa <ArrowUpRight /></Link></header><div className="focus-list">{focuses.map((focus, index) => <article key={focus.title}><span>0{index + 1}</span><h3>{focus.title}</h3><p>{focus.text}</p></article>)}</div></section>; }
